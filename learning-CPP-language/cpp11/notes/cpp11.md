...
#### 定义和初始化
##### 1.范式:vector<\类型> 标识符;(默认初始化;空vector)
##### 2.范式:vector<\类型> 标识符(另一个vector);(拷贝初始化)
##### 3.范式:vector<\类型> 标识符=另一个vector;(拷贝初始化)
> 拷贝初始化的类型要一致
##### 4.范式:vector<\类型> 标识符(n,值);(n个重复的值)
##### 5.范式:vector<\类型> 标识符(n);(n个默认初始化的值)
##### 6.范式:vector<\类型> 标识符{a,b,c...};(列表初始化)
##### 3.范式:vector<\类型> 标识符={a,b,c...};(列表初始化)
> 初始化多元素只能采用列表初始化

### 编写建议
#### 1. 访问字符串前,检查字符串是否为空或者索引是否越界
